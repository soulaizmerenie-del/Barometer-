...
STATUS_MARKERS: dict[str, tuple[str, ...]] = {
    ...
    "in_progress": ("в процессе", "делаю", "работаю", "начал", "занимаюсь", "уточняю",
                    "жду ответ", "в работе", "согласов"),
    "blocked": ("не могу", "нет ответа", "заблок", "проблема", "отказ", "сорвал",
                "не получилось", "нет в наличии", "задерж"),
...
def _mentions(task_title: str, text: str) -> int:
    ...
    task_words = {w[:6] for w in re.findall(r"[а-яa-zё]{5,}", task_title.lower().replace("ё", "е"))}
    msg_words = {w[:6] for w in re.findall(r"[а-яa-zё]{5,}", text.lower().replace("ё", "е"))}
    return len(task_words & msg_words)
